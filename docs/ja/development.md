...
Docs smoke / signal script を追加または rename した場合は、`node script/test_docs_entrypoint_suite.mjs --self-test` を実行し、suite registration が欠けていないことを確認します。この self-test は `npm run test:docs-entrypoints` 内の `Docs entrypoint suite option contract` group としても登録されています。candidate docs entrypoint script が suite の `checks` array または明示的な exclusion に入っているかを確認する registration guard です。明示的な exclusion は silent skip ではありません。excluded script は別の npm script または suite 経由で登録されている必要があり、新しい docs smoke / signal script は `checks` array に追加するか、登録経路と理由を持つ明示的な exclusion として文書化してください。CI-policy docs signal と importmap CI-policy routing は `npm run test:ci-policy`、Development docs command signal は `npm run test:development-docs-commands`、page-set parity は `npm run test:docs-i18n` が担当します。これらの alternate entrypoint があるため、対応 script を docs-entrypoint checks array から明示的に exclusion できます。candidate pattern や CI workflow を再設計するための入口ではありません。

`npm run test:entrypoints` の中では、`script/test_entrypoints.mjs` が runtime の package-root exports、manifest loader、`.d.ts` の export-name inventory を確認します。`script/test_controller_entries_contract.mjs` は別に、manifest-backed な `javascript_package_root.controller_registrations` と runtime の `TreeViewControllerEntries`、`registerTreeViewControllers(application)` が行う登録呼び出しを照合します。この smoke で Ruby-backed loader failure が出た場合は、Ruby が使える repository root から実行し、`javascript_package_root.controller_registrations` を確認してください。その後 `script/test_declaration_literal_shapes.mjs` が、event names、detail keys、remote-state values、transfer values、controller identifiers、selection data hooks、empty-state hooks などの manifest-backed JavaScript constants について、`app/javascript/tree_view/index.d.ts` の literal shape を確認します。package-root export が足りない、または余分な場合は export-name guard、identifier、controller reference、registration order がずれた場合は controller entries contract guard、export は存在するが key、tuple、代表 literal value が `config/public_api_manifest.yml` とずれた場合は literal-shape guard を見ます。これらは smoke guard であり、TypeScript compiler や declaration generator ではありません。
...
docs-entrypoint suite は、より狭い manifest-backed reader guidance を責務ごとに分けています。`Manifest-backed public surface signals` は public constants、helper methods、setup generator paths、event detail keys の broad sync を守ります。`Localized and hook docs signals` は localized-name lookup key family と、選択した remote-state / toolbar hook docs に焦点を当てます。`Render window and resource table docs signals` は `render_window_metadata` / `resource_table_render_state_call` と Windowed Rendering / Resource Table Bridge の reader-facing boundary を同期し、manifest shape は `script/test_public_api_manifest_structure.mjs` が担当します。

`Host app extension diagnostics signals` は Host App Extension Points から `TreeView::Diagnostics.run`、`validate_node_keys: true`、`RenderState#validate_unique_dom_ids!`、Tree diagnostics guide へ辿る reverse lookup を守り、cycle、orphan、duplicate node key guidance も対象にします。Tree diagnostics の accepted checks、run options、`Result` surface は `script/test_diagnostics_docs_signals.mjs` が引き続き担当し、host-app-extension guard は extension-point guide から diagnostics を発見できることを担当します。
...
PathTreeBuilder では、`path_tree_builder_node_shapes` が machine-readable な `FolderNode` / `RecordNode` の field / predicate surface を所有します。`spec/public_api_path_tree_builder_contract_spec.rb` はこれらの shape と runtime struct を比較し、`script/test_public_api_docs_signals.mjs` は英日 PathTreeBuilder guide の代表 wording を同期します。この manifest-backed node shape は folder key generation strategy、sorting algorithm、file-manager behavior、row action design を固定しません。それらは implementation / feature docs の behavior または host app の責務です。

Diagnostics では、`diagnostics.accepted_checks`、`diagnostics.run_options`、`diagnostics.result_surface` が machine-readable な accepted check 名、public run option key、`Result` surface を所有します。runtime validation behavior は `lib/tree_view/diagnostics.rb` と compatibility specs が担当し、`script/test_diagnostics_docs_signals.mjs` は manifest と英日 Tree diagnostics docs の代表 wording の同期だけを担当します。この docs signal は runtime execution を検証せず、individual error detail shape、orphan warning semantics、cycle validation policy を固定しません。
